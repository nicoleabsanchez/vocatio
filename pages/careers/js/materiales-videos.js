(function() {
  ...
  // Get all videos from DOM
  function getAllVideosFromDOM() {
    const videos = [];
    ...
    cards.forEach((card, index) => {
      ...
      videos.push({ id, title, description, image, type: 'video' });
    });
    return videos;
  }
  ...
  window.toggleFavorite = function(event, materialId) {
    ...
    if (index > -1) {
      ...
      const card = document.querySelector(`[data-video-id="${materialId}"]`) || 
    ...
    if (saved.length === 0) {
      savedGrid.innerHTML = `
        <div class="empty-state" style="grid-column: 1 / -1;">
          ...
          <h3>No hay videos guardados</h3>
          <p>Guarda videos para acceder a ellos posteriormente</p>
        </div>
      `;
      return;
    }
  ...
  window.removeFavorite = function(event, materialId) {
    ...
    confirmModal.innerHTML = `
      <div style="background: white; border-radius: 12px; padding: 32px; max-width: 400px; box-shadow: 0 20px 25px rgba(0,0,0,0.15);">
        ...
        <p style="margin: 0 0 24px 0; color: #6b7280;">¿Estás seguro de que deseas eliminar este video de guardados?</p>
  ...
  // Watch video
  window.watchVideo = function(title) {
